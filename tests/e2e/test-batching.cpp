...
int main(int argc, char ** argv) {
    ...
    // ---- 3. prefix caching: same prompt twice + shared long prefix ----
    {
        ...
        std::string t1, e1; uint32_t c1 = 0; auto r1 = collect(h1, t1, c1, e1);
        auto h2 = eng.submit(ta, sp);
        std::string t2, e2; uint32_t c2 = 0; auto r2 = collect(h2, t2, c2, e2);
        auto h3 = eng.submit(tok.encode(p2, true, true), sp);
        std::string t3, e3; uint32_t c3 = 0; auto r3 = collect(h3, t3, c3, e3);
        // reference without cache
        EngineConfig ec2 = ec; ec2.enable_prefix_caching = false;
        Engine eng2(model, ec2); eng2.start();
        auto hr = eng2.submit(ta, sp); std::string tr, er; uint32_t cr = 0; auto rr = collect(hr, tr, cr, er);
        auto hr3 = eng2.submit(tok.encode(p2, true, true), sp); std::string tr3, er3; uint32_t cr3 = 0; auto rr3 = collect(hr3, tr3, cr3, er3);
        eng2.stop();
        ...
        if (r1 != rr || r2 != rr) { printf("FAIL: cached outputs differ from reference\n  ref: %s\n  r1: %s\n  r2: %s\n", tr.c_str(), t1.c_str(), t2.c_str()); failures++; }
        if (r3 != rr3) { printf("FAIL: shared-prefix output differs from reference\n  ref: %s\n  got: %s\n", tr3.c_str(), t3.c_str()); failures++; }
        eng.stop();
    }
