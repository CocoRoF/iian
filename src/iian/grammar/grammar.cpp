...
static std::pair<uint32_t, const char *> parse_token(const Tokenizer * vocab, const char * src) {
    const char * pos = src;
    if (*pos != '<') {
    ...
    if (!vocab) {
        throw std::runtime_error("token literals require a tokenizer");
    }
    std::vector<token_t> tokens = vocab->encode(std::string_view(src, pos - src), /*add_special*/ false, /*parse_special*/ true);
    if (tokens.size() != 1) {
...
struct llama_grammar * llama_grammar_init_impl(
        ...
        for (const auto & elem : vec_rules[i]) {
            if (elem.type == LLAMA_GRETYPE_RULE_REF) {
                if (elem.value >= n_rules || vec_rules[elem.value].empty()) {
                    LOG_ERR("grammar", "invalid grammar: rule %zu references undefined rule %u\n", i, elem.value);
                    return nullptr;
                }
            }
        }
    ...
    for (size_t i = 0; i < n_rules; i++) {
        ...
        if (llama_grammar_detect_left_recursion(vec_rules, i, &rules_visited, &rules_in_progress, &rules_may_be_empty)) {
            LOG_ERR("grammar", "unsupported grammar, left recursion detected for nonterminal at index %zu", i);
            return nullptr;
        }
    }
...
struct llama_grammar * llama_grammar_init_impl(
    ...
    for (size_t i = 0; i < n_rules; i++) {
        ...
        if (llama_grammar_detect_left_recursion(vec_rules, i, &rules_visited, &rules_in_progress, &rules_may_be_empty)) {
            LOG_ERR("grammar", "unsupported grammar, left recursion detected for nonterminal at index %zu\n", i);
            return nullptr;
        }
    }
    ...
    std::vector<token_t>    vec_trigger_tokens;
...
const std::string & llama_grammar::piece(token_t token) const {
    if (pieces.empty()) {
        const int32_t n = vocab->n_tokens();
        pieces.resize(n);
        for (int32_t i = 0; i < n; i++) {
            pieces[i] = vocab->token_to_piece(i, /*special*/ true);
        }
    }
    return pieces[token];
}

void llama_grammar_apply_impl(const struct llama_grammar & grammar, float * logits, int32_t n_vocab) {
    ...
    for (int32_t id = 0; id < n_vocab; ++id) {
        if (logits[id] == -INFINITY) {
            continue;   // already excluded by an earlier processor
        }
        const std::string & piece = grammar.piece(id);
        ...
        if (grammar.vocab->is_eog(id)) {
            ...
            candidates_grammar.push_back({ (size_t) id, candidates_decoded.back().first.data(), candidates_decoded.back().second, id });
        }
    }

    const auto rejects = llama_grammar_reject_candidates(grammar.rules, grammar.stacks, candidates_grammar);
    for (const auto & reject : rejects) {
        logits[reject.index] = -INFINITY;
    }
}

bool llama_grammar_would_accept(const struct llama_grammar & grammar, token_t token) {
    GGML_ASSERT(grammar.vocab != nullptr);
    if (grammar.awaiting_trigger) {
        return true;
    }
    ...
    const std::string & piece = grammar.piece(token);
    if (piece.empty() || piece[0] == 0) {
        return false;
    }
    const auto decoded = decode_utf8(piece, grammar.partial_utf8);
    llama_grammar_candidates cand;
    cand.push_back({ 0, decoded.first.data(), decoded.second, token });
    return llama_grammar_reject_candidates(grammar.rules, grammar.stacks, cand).empty();
}

void llama_grammar_accept_impl(struct llama_grammar & grammar, token_t token) {
    GGML_ASSERT(grammar.vocab != nullptr);

    const auto & piece = grammar.piece(token);
    ...
    if (grammar.awaiting_trigger) {
        if (std::find(grammar.trigger_tokens.begin(), grammar.trigger_tokens.end(), token) != grammar.trigger_tokens.end()) {
            ...
            LOG_DBG("grammar", "Grammar triggered on token %u (`%s`)", token, piece.c_str());
            return;
        } else {
            ...
            for (const auto & trigger_pattern : grammar.trigger_patterns) {
                ...
                if (start != std::string::npos) {
                    ...
                    LOG_DBG("grammar", "Grammar triggered on regex: '%s'\n", constrained_str.c_str());
                    return;
                }
            }
            LOG_DBG("grammar", "Grammar still awaiting trigger after token %d (`%s`)\n", token, piece.c_str());
            return;
        }
    }
...
void llama_grammar_accept_token(struct llama_grammar & grammar, token_t token, const std::string & piece) {
    // Note terminating 0 in decoded string
    const auto   decoded     = decode_utf8(piece, grammar.partial_utf8);
    const auto & code_points = decoded.first;
