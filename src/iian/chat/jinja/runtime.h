...
#define JJ_DEBUG(msg, ...)  do { if (::iian::jinja::g_jinja_debug) printf("%s:%-3d : " msg "\n", FILENAME, __LINE__, __VA_ARGS__); } while (0)
...
inline std::vector<statement *> stmts_to_ptr(const statements & stmts) {
...
template<typename T>
inline void chk_type(const statement_ptr & ptr) {
...
inline void gather_string_parts_recursive(const value & val, value_string & parts) {
...
inline std::string render_string_parts(const value_string & parts) {
