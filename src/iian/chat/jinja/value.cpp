...
static std::string json_ensure_ascii_preserving_format(const std::string & json_str) {
    ...
    for (size_t pos = 0; pos < json_str.size();) {
        ...
        auto parsed = parse_utf8_codepoint(json_str, pos);
...
static value from_json(const T_JSON & j, bool mark_input) {
    if (j.is_null()) {
        return mk_val<value_none>();
    } else if (j.is_boolean()) {
        return mk_val<value_bool>(j.template get<bool>());
    } else if (j.is_number_integer()) {
        return mk_val<value_int>(j.template get<int64_t>());
    } else if (j.is_number_float()) {
        return mk_val<value_float>(j.template get<double>());
    } else if (j.is_string()) {
        auto str = mk_val<value_string>(j.template get<std::string>());
        if (mark_input) {
            str->mark_input();
        }
        return str;
...
// explicit instantiations for the two nlohmann flavours (ordered_json keeps key insertion order)
template void global_from_json<nlohmann::json>(context & ctx, const nlohmann::json & json_obj, bool mark_input);
template void global_from_json<nlohmann::ordered_json>(context & ctx, const nlohmann::ordered_json & json_obj, bool mark_input);
