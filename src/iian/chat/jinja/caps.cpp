...
std::map<std::string, bool> caps::to_map() const {
    return {
        ...
        {"supports_tool_responses", supports_tool_responses},
...
caps caps_get(jinja::program & prog) {
    ...
    caps_try_execute(
        ...
        [&](context &, bool success, value & messages, value & tools, const std::string &) {
            ...
            auto & tool_response = messages->at(2)->at("content");
            caps_print_stats(tool_response, "messages[2].content");
            if (tool_response->stats.used) {
                result.supports_tool_responses = true;
            }
        }
    );
    ...
    JJ_DEBUG("%s\n", ">>> Running capability check: enable_thinking");

    // case: enable_thinking variable is referenced by the template
    caps_try_execute(
        prog,
        [&]() {
            // messages
        ...
        nullptr, // tools_fn
        [&](context & ctx, bool, value &, value &, const std::string &) {
            value enable_thinking = ctx.get_val("enable_thinking");
            caps_print_stats(enable_thinking, "enable_thinking");
            result.supports_enable_thinking = enable_thinking->stats.used;
        }
    );
