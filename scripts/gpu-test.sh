...
# multi-GPU: layer split across every visible GPU (auto share, plus a lopsided explicit split), same equivalence tests
NGPU=$(nvidia-smi --query-gpu=index --format=csv,noheader | wc -l)
if [ "$NGPU" -ge 2 ]; then
  ALL=$(nvidia-smi --query-gpu=index --format=csv,noheader | sed 's/^/CUDA/' | paste -sd,)
  run "multi-GPU batching ($ALL)"   sh -c "IIAN_DEVICES=$ALL ./build/tests/test-batching $M 2>&1 | grep -E '$B'"
  run "multi-GPU draft, split 3,1"  sh -c "IIAN_DEVICES=$ALL IIAN_TENSOR_SPLIT=3,1 ./build/tests/test-batching $M auto 0 models/SmolLM2-135M-Instruct-Q8_0.gguf 2>&1 | grep -E '$B'"
fi
