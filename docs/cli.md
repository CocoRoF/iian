...
| Engine / KV cache | `--spec-ngram N` (prompt-lookup speculative decoding, 0 = off) `--spec-draft MODEL --spec-draft-n N` (draft-model speculation; same tokenizer) `--attention auto|masked|paged|gather` (auto = paged kernel on CPU, gathered batched flash attention on GPU) `--no-repack` `-c/--max-model-len N` (default: min(training ctx, 8192)) `--kv-cache-tokens N` `--kv-cache-gb G` `--kv-dtype f16|bf16|f32|q8_0|q4_0|q4_1|q5_0|q5_1` `--block-size N` (16) `--no-prefix-caching` `--no-flash-attn` `--seed N` |
